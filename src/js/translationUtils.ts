...
import { routeTranslations, localizedCollections } from "@config/routeTranslations";
...
async function generateDynamicRouteTranslations(): Promise<RouteMap> {
  ...
  for (const [collectionName, collectionLocales] of Object.entries(localizedCollections)) {
    ...
    for (const entry of allEntries) {
      ...
      // entry.id is like "en/first-post-in-english.md" or "fr/premier-article-en-francais.md"
      const idParts = entry.id.split("/");
      const entryLocale = idParts[0] as Locale;
      if (!locales.includes(entryLocale)) continue;

      // Get slug without locale prefix and extension
      const slug = idParts.slice(1).join("/").replace(/\.\w+$/, "");
