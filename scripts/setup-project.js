...
const featuresFile = join(root, "src", "features", "featuresFlags.ts");
...
function readFeatureFlags() {
  ...
  const getFlag = (name) => {
    const match = content.match(new RegExp(`${name}\\s*:\\s*(true|false)`));
...
function runScript(scriptName) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(process.execPath, [join(root, "scripts", `${scriptName}.js`)], {
      stdio: "inherit",
    });
...
async function main() {
  ...
  for (const feature of FEATURES) {
    ...
    const keep = await askYesNo(ask, `Keep ${feature.label}?`);
  ...
  if (i18nAction && !i18nAction.remove) {
    configLocalesNow = await askYesNo(ask, "\nWould you like to configure your locales now?");
  }

  console.log("\nSummary\n");

  for (const action of actions) {
    console.log(`${action.remove ? "✗ Remove" : "✓ Keep"} ${action.label}`);
  }

  const proceed = await askYesNo(ask, "\nProceed with these changes?");
